...
export default (router, { services, exceptions }) => {
	...
	router.post('/register', async (req, res, next) => {
		if (req.body.email == null || req.body.password == null) {
			return res.status(400).json({ status: 400, status_text: 'Please provide email & password fields.' });
		}
		const email = `${req.body.email}`.toLowerCase();
		if (!/[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?/gi.test(email)) {
			return res.status(400).json({ status: 400, status_text: 'Email field is invalid.' });
		}
		if (`${req.body.first_name || ''}`.length < 2) {
			return res.status(400).json({ status: 400, status_text: 'First name field has to be 2 or more characters long.' });
		}
		if (`${req.body.last_name || ''}`.length < 2) {
			return res.status(400).json({ status: 400, status_text: 'Last name field has to be 2 or more characters long.' });
		}
		if (`${req.body.phone || ''}`.length < 11) {
			return res.status(400).json({ status: 400, status_text: 'Phone field has to be 11 or more characters long.' });
		}
		if (`${req.body.password}`.length < passMinLength) {
			return res.status(400).json({ status: 400, status_text: `Password should be ${passMinLength} or more characters long.` });
		}
		...
		try {
			...
			return res.status(500).json({ status: 500, status_text: err.toString() });
		}
		...
		return res.status(201).json({ status: 201, access_token: authData.accessToken, expires: authData.expires });
	});
	...
	router.post('/purchase/:course_id', async (req, res, next) => {
		...
		if (account == null || account.user == null) {
			return res.status(401).json({ status: 401, status_text: 'User has to be authorized.' });
		}
		...
		if (paymentData != null && paymentData.status === 'paid') {
			return res.status(500).json({ status: 500, status_text: 'Course is already paid' });
		}
